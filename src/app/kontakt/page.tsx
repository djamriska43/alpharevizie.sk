      ...
      <section className="py-20 px-8">
        <div className="max-w-[1200px] mx-auto grid grid-cols-1 md:grid-cols-2 gap-12">
          <div className="flex flex-col gap-6">
            {[
              {icon:'☎',title:'Telefón',content:'<a href="tel:+421952352669" class="text-black font-semibold no-underline">+421 952 352 669</a>'},
              {icon:'✉',title:'E-mail',content:'<a href="mailto:office@alpharevizie.sk" class="text-black font-semibold no-underline">office@alpharevizie.sk</a>'},
              {icon:'📍',title:'Adresa',content:'Jeleňova 4154/9<br/>036 01 Martin'},
              {icon:'🏢',title:'Fakturačné údaje',content:'Alpha Safety s.r.o.<br/>IČO: 54 610 982<br/>DIČ: 2121731810<br/>IČ DPH: SK2121731810'},
            ].map((item,i) => (
          ...
          <Reveal direction="right">
            <form className="flex flex-col gap-4" onSubmit={(e)=>{e.preventDefault();alert('Formulár odoslaný! (demo)')}}>
              ...
              <select required className="w-full py-4 px-5 border border-gray-200 rounded-xl font-sans text-sm outline-none focus:border-black transition-colors bg-white">
                ...
                <option>Elektrické zariadenia</option><option>Plynové zariadenia</option><option>Tlakové zariadenia</option><option>Zdvíhacie zariadenia</option><option>Iné</option>
              </select>
