    ...
    <>
      ...
      <section className="min-h-screen bg-[#111] relative overflow-hidden">
        ...
        <div className="max-w-[1400px] mx-auto grid grid-cols-1 md:grid-cols-2 gap-8 items-center px-8 pt-36 pb-20 relative z-10">
          <div>
            <h1 className="text-[clamp(2.8rem,5vw,4.2rem)] font-black leading-[1.05] tracking-tighter mb-6 text-white animate-fadeUp" style={{animationDelay:'0.1s'}}>
              Revízie technických zariadení po celom Slovensku
            </h1>
            <p className="text-lg text-gray-400 leading-relaxed max-w-[500px] mb-10 animate-fadeUp" style={{animationDelay:'0.2s'}}>
      ...
      {/* COUNTERS */}
      <section className="bg-[#111] py-16 px-8 border-t border-white/5">
            ...
            {whyItems.map((item, i) => (
              <Reveal key={i} delay={i * 0.1}>
                <div className={`text-center p-8 rounded-2xl border border-gray-200 hover:-translate-y-1 hover:shadow-[0_15px_40px_rgba(0,0,0,0.06)] transition-all group glow-icon-${item.c}`}>
                  <div className="w-16 h-16 rounded-full flex items-center justify-center text-2xl mx-auto mb-5 transition-shadow duration-300 glow-target" style={{background:bgColors[item.c]}}>{item.icon}</div>
