      ...
      <section className="min-h-screen grid grid-cols-1 md:grid-cols-2 gap-8 items-center px-8 pt-32 pb-20 relative overflow-hidden max-w-[1400px] mx-auto">
        ...
        <div className="grid grid-cols-2 gap-4 relative z-10 animate-fadeUp" style={{ animationDelay: '0.3s' }}>
          {svcKeys.map((key) => {
            ...
            return (
              <Link key={key} href={`/${s.slug}`} className="bg-white border border-gray-200 rounded-2xl p-6 no-underline text-black cursor-pointer transition-all duration-300 relative overflow-hidden group hover:-translate-y-1 hover:border-transparent" style={{ ['--glow' as string]: glowColors[key] }}>
                ...
                <style jsx>{`.group:hover { box-shadow: 0 15px 40px var(--glow); }`}</style>
              </Link>
            );
          })}
        </div>
            ...
            {svcKeys.map((key, i) => {
              ...
              return (
                <Reveal key={key} delay={i * 0.1}>
                  <Link href={`/${s.slug}`} className="block bg-white/[0.04] rounded-2xl p-8 border border-white/[0.08] no-underline transition-all duration-400 relative overflow-hidden group cursor-pointer hover:-translate-y-1.5 hover:border-transparent" style={{ ['--glow' as string]: glowColors[key] }}>
                    ...
                    <style jsx>{`.group:hover { box-shadow: 0 20px 50px var(--glow); }`}</style>
                  </Link>
                </Reveal>
              );
            })}
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mt-14">
            {[
              ...
              <Reveal key={i} delay={i * 0.1}>
                <div className="text-center p-8 rounded-2xl border border-gray-200 hover:-translate-y-1 hover:shadow-[0_15px_40px_rgba(0,0,0,0.06)] transition-all group">
                  <div className="w-16 h-16 rounded-full flex items-center justify-center text-2xl mx-auto mb-5 transition-shadow duration-300 group-hover:shadow-[0_0_25px_var(--glow)]" style={{ background: bgColors[item.c as keyof typeof bgColors], ['--glow' as string]: glowColors[item.c as keyof typeof glowColors] }}>{item.icon}</div>
